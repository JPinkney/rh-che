...
public class KeycloakAuthenticationFilter extends org.keycloak.adapters.servlet.KeycloakOIDCFilter {
  ...
  @Inject private KeycloakUserChecker userChecker;

  @Inject private ServiceAccountInfoProvider serviceAccountInfoProvider;
  ...
  public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
    ...
    if (isSystemStateRequest(requestURI)
        || isWebsocketRequest(requestURI, requestScheme, upgradeHeader)
        || isKeycloakSettingsRequest(requestURI)
        || isWorkspaceAgentRequest(authHeader)
        || isRequestFromGwtFrame(requestURI)
        || isStackIconRequest(requestURI)
  ...
  /**
   * @param requestURI
   * @return true if request is made against system state endpoint which is used in OpenShift
   *     liveness & readiness probes, false otherwise
   */
  ...
  /**
   * @param requestURI
   * @return true if request is made against endpoint which provides keycloak status (enabled /
  ...
  /**
   * @param authHeader
   * @return true if "Authorization" header has valid openshift token that is used for communication
   *     between wsagent and wsmaster, false otherwise
   */
  ...
  private boolean isWebsocketRequest(
      String requestURI, String requestScheme, String upgradeHeader) {
    return "websocket".equals(upgradeHeader)
        && //
        (requestURI.endsWith("/ws")
            || requestURI.endsWith("/eventbus")
            || requestScheme.equals("ws")
            || requestScheme.equals("wss")
            || requestURI.contains("/websocket")
            || requestURI.endsWith("/wsagent"));
  }

  /**
   * OpenShift default service account token is used in "Authorization" header for communication
   * between wsagent and wsmaster. The method checks if the token is valid by fetching namespace
   * info and comparing it with expected service account namespace
   *
   * @param token
  ...
  /**
   * @param requestURI
   * @return true if request comes from a GWT `Frame`, and thus cannot contain the Keycloak token,
   *     false otherwise
   *     <p>There a currently 2 places in the Java Che support that use `Frame` objects to display
   *     documentation popups
   *     <p>see issue https://github.com/redhat-developer/rh-che/issues/94
   */
  private boolean isRequestFromGwtFrame(String requestURI) {
    try {
      ...
      return "/api/java/javadoc/find".equals(uri.getPath()) // Java Quick Documentation popup
          || "/api/java/code-assist/compute/info"
              .equals(uri.getPath()); // Java content assist Documentation popup
