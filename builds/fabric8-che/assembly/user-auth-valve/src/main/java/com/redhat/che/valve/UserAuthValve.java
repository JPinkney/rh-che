...
/**
 * Performs Keycloak and OpenShift.io user validation. Prompts user to login if necessary and checks
 * if the logged in user has access to the current Che instance.
 *
...
public class UserAuthValve extends KeycloakAuthenticatorValve {
  ...
  /**
   * Verify that a logged in user has access to the current project by making a request against the
   * /api/token/user endpoing on Che server.
   *
   * <p>Base URL for the request is set to the default service name for Eclipse Che ({@code
   * che-host})
   *
